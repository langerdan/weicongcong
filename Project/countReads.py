...
import sys
import xlwt
from Lib.BASE import get_file_path

def readPrimerDetails(p_file):
    amplicon_details = {}
    with open(p_file, 'rb') as r_obj:
        for line_no, line in enumerate(r_obj):
            if line_no == 0:
                continue
            amplicon_name = re.match('(?:[^\t]+\t){11}([^\t]+)', line).group(1)
            ...
            insert_start = int(re.match('(?:[^\t]+\t){7}([^\t]+)', line).group(1))
            insert_end = int(re.match('(?:[^\t]+\t){8}([^\t]+)', line).group(1))
            amplicon_start = int(re.match('(?:[^\t]+\t){6}([^\t]+)', line).group(1))
            amplicon_end = int(re.match('(?:[^\t]+\t){9}([^\t]+)', line).group(1))
            amplicon_details[amplicon_name] = [amplicon_start, amplicon_end, insert_start, insert_end]
    return amplicon_details

def saveStat(y_data1, y_data2, x_axis):
    def setStyle(height=210, bold=False, color_index=xlwt.Style.colour_map['black'], name='Microsoft YaHei UI'):
        style = xlwt.XFStyle()
        font = xlwt.Font()
        font.name = name
        font.bold = bold
    ...
    for (row_no, row_data) in enumerate(y_data1):
        sheet1.write(row_no + 1, 0, x_axis[row_no], setStyle(220, True))
        for (key, value) in a_details_sorted:
            sheet1.write(0, a_details_sorted.index((key, value)) + 1, key, setStyle(220, True))
            sheet1.write(row_no + 1, a_details_sorted.index((key, value)) + 1, row_data[key])
    for (row_no, row_data) in enumerate(y_data2):
        sheet2.write(row_no + 1, 0, x_axis[row_no], setStyle(220, True))
        for (key, value) in a_details_sorted:
            sheet2.write(0, a_details_sorted.index((key, value)) + 1, key, setStyle(220, True))
...
a_details = readPrimerDetails(path_primer_details)
print 'OK!'
amplicon_stat_am = []
amplicon_stat_in = []
...
saveStat(amplicon_stat_am, amplicon_stat_in, sam_basename_list)
